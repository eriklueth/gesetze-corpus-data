---
schema_version: v2
bjnr: BJNR180910013
jurabk: EUAHiG
type: paragraph
number: "§ 13"
heading: Zustellungsersuchen an andere Mitgliedstaaten
breadcrumb:
  - Abschnitt 1 Allgemeine Bestimmungen
  - Abschnitt 4 Sonstige Formen der Verwaltungszusammenarbeit
stand_datum: "2025-12-22"
source_xml: source.xml
---

# § 13 Zustellungsersuchen an andere Mitgliedstaaten

(1) Auf Ersuchen der zuständigen Finanzbehörde beantragt das zentrale Verbindungsbüro bei einem anderen Mitgliedstaat die Zustellung von Dokumenten und Entscheidungen der Finanzbehörde, die mit einer Steuer nach § 1 zusammenhängen.

(2) Ein Zustellungsersuchen ist nur dann zulässig, wenn 1. die Finanzbehörde nicht in der Lage ist, die Zustellung nach den Vorschriften des Verwaltungszustellungsgesetzes im anderen Mitgliedstaat vorzunehmen, oder 2. die Zustellung mit unverhältnismäßig großen Schwierigkeiten verbunden wäre.

(3) Im Zustellungsersuchen ist Folgendes anzugeben: 1. der Gegenstand des zuzustellenden Dokuments oder der zuzustellenden Entscheidung, 2. der Name und die Anschrift des Adressaten sowie 3. alle weiteren Informationen, die die Identifizierung des Adressaten erleichtern können.

(4) Einer in einem anderen Mitgliedstaat ansässigen Person kann jedes Dokument per Einschreiben oder auf elektronischem Weg direkt zugestellt werden.

(5) Das zentrale Verbindungsbüro leitet Informationen über veranlasste Zustellungen anderer Mitgliedstaaten den Finanzbehörden, die die Informationen verwenden, weiter.
